...
import Text.Parsec (Parsec, ParseError, choice, try, parse, letter, many, many1, eof, char, string, sepBy1, digit, optionMaybe, (<|>))
...
hexStringParser = do
  _ <- string "0x"
  val <- many1 (choice [digit, char 'a', char 'b', char 'c', char 'd', char 'e', char 'f'])
  case hexadecimal (pack ("0x" ++ val)) of
    Right (x, r) -> if (not . Data.Text.null $ r) then (fail "Not a hex number") else return x
...
floatImmediateParser = do
  ...
  postPeriod <- optionMaybe (do
    _ <- char '.'
    many1 digit)
  return FloatImmediate {fvalue=read (prePeriod ++ (if isJust postPeriod then ("." ++ fromJust postPeriod) else ""))}
...
operandParser = choice [try registerParser, try predicateRegisterParser, try truePredicateParser, falsePredicateParser, zeroRegisterParser, specialRegisterParser, try uniformRegisterParser, try uniformZeroRegisterParser, try uniformTruePredicateParser, uniformFalsePredicateParser, barrierParser, try immediateParser, floatImmediateParser, constantMemoryParser, effectiveAddressParser, negativeParser, negateParser, absoluteParser, tildeParser, textAddressParser, positiveInfinityParser]
