...
instance Show Instruction where
  show Instruction { predicate=pdc, op=o, operands=opnds } = predStr ++ o ++ argStr
    where
      predStr = case pdc of
        Just x -> (show x) ++ " "
        Nothing -> ""
...
instance Show Operand where
  ...
  show EffectiveAddress {address=addr, offset=offs} = "[" ++ (show addr) ++ (if isJust offs then ("+" ++ (show . fromJust $ offs)) else "") ++ "]"
  ...
  show ConstantMemory {arg1=i1, arg2=i2} = "c[" ++ (show i1) ++ "][" ++ (show i2) ++ "]"
